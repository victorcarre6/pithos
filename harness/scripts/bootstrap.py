...
def inspect(harness_root: Path, repository_root: Path, logs_root: Path) -> dict:
    """Return the local bootstrap state without changing it."""

    # fichiers contrôlés et commandes hôte
    missing_files = [path for path in REQUIRED_PROJECT_FILES if not (harness_root / path).is_file()]
    missing_commands = [command for command in COMMANDS if shutil.which(command) is None]

    # emplacements persistants
    expected_directories = [repository_root / path for path in PROJECT_DIRECTORIES]
...
def bootstrap(harness_root: Path, repository_root: Path, logs_root: Path) -> dict:
    """Create only missing directories and the append-only live log."""

    # validation avant toute écriture
    state = inspect(harness_root, repository_root, logs_root)
    if state["missing_files"]:
        missing = ", ".join(state["missing_files"])
        raise RuntimeError(f"missing controlled project files: {missing}")

    for config_path in (harness_root / "config").glob("*/models.json"):
        json.loads(config_path.read_text(encoding="utf-8"))
    for config_path in (harness_root / "config").glob("*/settings.json"):
        json.loads(config_path.read_text(encoding="utf-8"))
    ...
    return inspect(harness_root, repository_root, logs_root)
...
def main() -> int:
    ...
    parser.add_argument("--project-root", dest="harness_root", type=Path, default=Path(__file__).parents[1])
    ...
    harness_root = arguments.harness_root.expanduser().resolve()
    repository_root = arguments.repository_root.expanduser().resolve()
    logs_root = arguments.logs_root.expanduser().resolve()
    state = (
        inspect(harness_root, repository_root, logs_root)
        if arguments.check
        else bootstrap(harness_root, repository_root, logs_root)
    )
