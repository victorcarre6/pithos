...
```bash
docker build -t pithos-agent:local runtime/agent
PITHOS_LOGS_ROOT="$HOME/logs/pithos" docker compose -f runtime/docker-compose.yml up -d
```

Le runner Docker utilise `config/pi-docker/`, dont l'URL Ollama cible `host.docker.internal`. La configuration
